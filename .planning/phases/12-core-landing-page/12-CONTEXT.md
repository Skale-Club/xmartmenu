...
- **D-03:** `src/app/(marketing)/layout.tsx` sets `<html lang="en">` (landing page is English), uses Inter with `display: 'swap'`, sets page-level metadata (`title`, `description`, `metadataBase`, `openGraph`, `twitter`). Analytics components (`<Analytics />`, `<SpeedInsights />`) are installed in the **root** `src/app/layout.tsx` (not the marketing layout) — this gives broader coverage across all routes including marketing. Import from `/next` subpaths (`@vercel/analytics/next`, `@vercel/speed-insights/next`).
