...
**v1.2 Phase 11 complete (2026-05-07)** — Menu photo OCR operational in superadmin panel.
- GPT-4.1-mini vision extracts categories, products, prices, and descriptions from a menu photo
- Two-step upload: signed Supabase URL → browser PUT (bypasses Vercel 4.5 MB limit) → POST storage path
- Additive writes; price=0 for unreadable prices; `ai_usage` tracks `ocr_menu` calls
- Requires: `OPENAI_API_KEY` in `.env.local`; `@ai-sdk/openai` now installed

**v1.2 Phase 10 complete (2026-05-07)** — AI image seeding operational in superadmin panel.
- Gemini 3 Pro Image (Nano Banana 2, `gemini-3.1-flash-image-preview`) generates cover + per-product photos
- `convertBufferToWebP()` sibling in `upload.ts` handles AI-generated bytes; separate `/seed-image` route with `maxDuration=300`
- AI Tools section: "Seed cover", "Seed product images", per-product category+product selector + "Seed image"
- Additive: skips if `banner_url`/`image_url` already set; `ai_usage` tracks `image_cover`/`image_product` calls
- Requires: same `GOOGLE_GENERATIVE_AI_API_KEY` from Phase 9; no new env vars
...
- [x] AI image seeding: cover photo + per-product photos via Nano Banana 2 (Gemini 3 Pro Image) (Phase 10)
- [x] Menu photo OCR: GPT-4.1-mini vision → categories/items/prices → direct DB write (Phase 11)
