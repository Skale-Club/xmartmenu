# Requirements Archive: v1.5 Image Optimization
...
- [x] **IMG-02**: Upload de logo e banner via superadmin (`upload/route.ts`) passa pelo conversor WebP — arquivo bruto não atinge o Storage
- [x] **IMG-03**: `next.config.ts` declara `formats: ['image/webp']` e `deviceSizes` alinhados com os tamanhos emitidos pelo conversor

## Rendering Admin

- [x] **IMG-04**: `BrandingClient.tsx` — logo e banner preview usam `next/image` com `width`/`height` ou `fill` + `sizes` corretos
- [x] **IMG-05**: `ProductsClient.tsx` — grade de produtos admin usa `next/image` com `sizes` de grid correto
- [x] **IMG-06**: `TenantsClient.tsx` e `TenantDetailClient.tsx` — logos de tenant no superadmin usam `next/image`
...
| IMG-04 | Phase 19 | Complete |
| IMG-05 | Phase 19 | Complete |
| IMG-06 | Phase 19 | Complete |
...
| IMG-07 | Phase 20 | Complete |
| IMG-08 | Phase 20 | Complete |
| IMG-09 | Phase 20 | Complete |
